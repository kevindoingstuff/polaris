...
4. Body, in this order:
   ...
   - `## Changes`: the summary diagram, then the change graph. Write `<tmp>/pr-summary.json`: `{"title": <the PR title>, "commits": [{"sha", "subject", "changes": ["Add …", …]}]}` — large PR: `"groups": [{"title", "commits": [shas], "changes": […]}]` instead. One chip per change, ≤6 words, first word Add, Change, Fix, or Remove. The chips are the record of the PR: every change to behaviour, an interface, a dependency, a file's role, or a default goes in; a change is never dropped to keep a column short. Run `prdiagram.py summary <tmp>/pr-summary.json -o <tmp>/pr-summary.drawio --png` and write `<!-- attach pr-summary.drawio.png -->` (the `.drawio` when there is no `.png`). Then the change graph: the step 2 output in a code block, and after each file ` — <what changed>`, ≤6 words, when the file name does not say it — large PR: run `prdiagram.py changes . <base> HEAD --groups <tmp>/pr-summary.json` and paste its `<details>` blocks as they are.
