...
| [`docstring`](docstring/SKILL.md) | Reconciles Python docstrings with the code, in PEP 257 and Google style, and keeps only the interface contract. It removes implementation narration, prose types that repeat the annotations, and changelog cruft; it writes the docstrings that are missing; and it corrects a claim the code does not support or adds a fact the docstring does not state. | `/docstring`, or say "clean up these docstrings" |
